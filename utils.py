...
class PreProcess(): #abstract base class for pre-processing stages
...
class PostProcess(): #abstract base class for post-processing stages
    __metaclass__ = abc.ABCMeta
...
class Essential_PreProcess(PreProcess):
    ...
    def apply(self, data: list):
        ...
        for i in range(len(data)):
            ...
            temp = re.sub('[^a-zA-Z]',' ',temp) #noise removal (removes all HTML tags, punctuation, etc. - anything that's not letters are removed)
            temp = ' '.join(temp.split()) #remove extra whitespace
            words = temp.split(" ")

            review = [word.lower() for word in words if not word == ""] #data lowercased
            review = ' '.join(review)
            data[i][1] = review
...
class Pipeline:
    ...
    def __order(self, arg):
        '''
        Simple Dependency resolver - by Sandesh Gade
    ...
    def __read_sentence(self): #for interactive mode
    ...
    def run(self): 
        if not self._interactive:
            ...
            while True:
                ...
                if self._data[0][1] == 'EXIT': #if 'EXIT', break
    ...
    def __execute_rules(self):
        for rule in self._rules:
            if rule._enabled: 
                print(rule)
                self._pred[f'{rule}'] = rule.execute(self) #just pass self object, all rules need different things
                #all except random forest needs rawdata
                #only negators need pred (predictions from rules they are based on)
    ...
    def __post_process(self):
        ...
        for stage in self._postprocess:
            if stage._enabled:
                output[stage] = stage.apply(self) #just pass self object
        return output
